...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="priorVersion" select="/rdf:RDF/rdf:Description[1]/owl:priorVersion"/>
  ...
  <xsl:template match="/">
    <html>
      ...
      <body>
        ...
        <xsl:if test="$priorVersion and not($priorVersion = '')">
          <div class="version">Prior Version: <a><xsl:attribute name="href"><xsl:value-of select="$priorVersion/@rdf:resource"/></xsl:attribute><xsl:value-of select="$priorVersion/text()"/></a></div>
        </xsl:if>
        <xsl:if test="$seeAlso != ''">
